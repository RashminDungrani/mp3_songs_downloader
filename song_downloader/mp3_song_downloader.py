...
Date:   21-Jan-2020
Discription: This Python Script can Download song from mp3paw website
"""
...
import traceback
import signal
from os import path, system as os_system, kill as os_kill, getppid as os_getppid
from song_downloader.utils import check_download_path_available, set_gecko_path, Paths
...
def strat_browser():
    ...
    profile.set_preference("browser.download.dir", Paths.download_path)
    profile.set_preference("browser.download.useDownloadDir", True)
    profile.set_preference("browser.helperApps.neverAsk.saveToDisk", "audio/mpeg")
    global browser
    browser = webdriver.Firefox(profile, firefox_options=options, executable_path=Paths.gecko_path)
...
def song_downlaoder(songName):
    ...
    mp3_file_name = path.join(Paths.download_path, songName)
    print('file Name -> ' + mp3_file_name)
    
    # check download start
    while not path.exists(mp3_file_name + '.part'):
        sleep(1)
    ...
    while path.exists(mp3_file_name + '.part'):
        sleep(1)

    if path.exists(mp3_file_name):
...
def run():
    try:
        if check_download_path_available():
            set_gecko_path()
